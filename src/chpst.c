...
int main(int argc, const char **argv)
{
        ...
        if (str_equal(progname, "setuidgid"))
                setuidgid(argc, argv);
        else if (str_equal(progname, "envuidgid"))
                envuidgid(argc, argv);
        else if (str_equal(progname, "envdir"))
                envdir(argc, argv);
        else if (str_equal(progname, "pgrphack"))
                pgrphack(argc, argv);
        else if (str_equal(progname, "setlock"))
                setlock(argc, argv);
        else if (str_equal(progname, "softlimit"))
                softlimit(argc, argv);

#define MAIN_OPTSTR "u:U:b:e:m:d:o:p:f:c:r:t:/:n:l:L:vP012V"

        while ((opt = getopt(argc, argv, MAIN_OPTSTR)) != opteof) {
                switch(opt) {
                case 'u':
